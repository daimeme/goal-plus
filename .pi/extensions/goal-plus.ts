...
const ToolizationExclusion = Type.Union([
	...
	Type.Literal("no_toolizable_material"),
...
const RuntimeToolDescriptions: Record<string, string> = {
	...
	search_get_agent_context:
		"读取当前 worker 的权威 candidate 上下文。candidate_task.share_out_dir 非空表示已启用 shared_dir，并返回不含路径和源码的 tool_family_catalog；catalog 用 revision_head 指定唯一更新基线，并用 revision_allowed、published_version_count 和 max_published_versions_per_family 暴露容量。toolization_decision 使用 repeated_workflow、domain_construction_or_probe、behavior_or_invariant_checker、reproducer_fixture_or_case_generator、parser_trace_or_comparator、peer_setup_or_feedback_reduction。搜索期测试文件，以及 candidate 自己编写并纳入最终交付的正式回归测试、功能验证函数、复现、fixture/case、差分和不变量检查可以工具化；测试框架、test_ 文件名、属于正式测试或验证同一目标行为都不是 restricted_artifact 理由。发布时仍须把显式选择的测试及依赖复制到 .tmp/tool-drafts/，并确保它能在 peer workspace 运行且不依赖 candidate 私有临时状态。candidate 产品实现、冻结 verifier/runner/grader、隐藏答案/评分逻辑、日志、原始数据、凭据和构建输出仍受限；共享测试不得复制、代理或近似重建隐藏反馈。短小、任务专属、来自 inline code 或只输出退出码都不是排除理由。首次工具发布保持低门槛。no_toolizable_material 表示累计回顾后确实没有工具。family 更新仅在 revision_allowed=true 时引用 revision_head.tool_id：capability_extension 必须新增稳定 capability/coverage key；adoption_fix 必须有同 family 的真实 copy/adoption 事实和具体缺陷；contract_change 必须为有价值的入口、输入、输出或依赖变化新增稳定契约键。不得靠改名或同义键制造增量。revision_allowed=false 时使用 revision_blocker 的准确值；existing_family_sufficient、no_material_tool_delta 和 max_published_versions_reached 是 revision-only 排除项，runtime 会在 catalog 尚无 family 时拒绝；existing_family_sufficient 仅表示 revision_head 已覆盖需求；no_material_tool_delta 仅表示已有 family 没有上述实质增量；draft_not_ready 只用于具体的安全性、完整性、可移植性或 peer 可运行性阻塞。",
